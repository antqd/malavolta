...
export default function UsatiList() {
  ...
  const load = async () => {
    ...
    try {
      const url = q ? `/api/trattori/usati?q=${encodeURIComponent(q)}` : "/api/trattori/usati";
  ...
  const onDelete = async (id:number) => {
    ...
    const r = await fetch(`/api/trattori/usati/${id}`, { method:"DELETE" });
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center gap-2">
        ...
        <Link href="/admin/trattori/usati/new" className="ml-auto rounded bg-emerald-600 text-white px-3 py-2">+ Nuovo</Link>
      </div>
              ...
              <div className="flex-1">
                ...
                <div className="mt-2 flex justify-between text-sm">
                  ...
                  <div className="space-x-3">
                    <Link href={`/admin/trattori/usati/${p.id}`} className="text-blue-600">Modifica</Link>
