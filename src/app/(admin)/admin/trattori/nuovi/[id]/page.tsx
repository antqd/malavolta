...
export default function NuoviEdit() {
  ...
  const load = async () => {
    const r = await fetch(`/api/trattori/nuovi/${id}`); const data = await r.json();
  ...
  const submit = async (e:React.FormEvent) => {
    ...
    try{
      ...
      const r = await fetch(`/api/trattori/nuovi/${id}`, { method:"PATCH", headers:{ "Content-Type":"application/json" }, body: JSON.stringify(body) });
  ...
  return (
    <div className="max-w-2xl">
      <h1 className="text-2xl font-semibold mb-4">Modifica (nuovi) #{id}</h1>
