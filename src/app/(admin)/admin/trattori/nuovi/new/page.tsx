...
export default function NuovoNew() {
  const router = useRouter();
  const [form, setForm] = useState<any>({ name:"", photo:"", description:"", price:"", quantity:0 });
  ...
  const submit = async (e:React.FormEvent)=> {
    ...
    try{
      ...
      const r = await fetch("/api/trattori/nuovi", { method:"POST", headers:{ "Content-Type":"application/json" }, body: JSON.stringify(body) });
      const data = await r.json();
      if(!r.ok) throw new Error(data?.error || data?.raw || "Errore");
      router.push("/admin/trattori/nuovi");
    }catch(e:any){ setErr(e.message);} finally{ setSaving(false); }
  };

  return (
    <div className="max-w-2xl">
      <h1 className="text-2xl font-semibold mb-4">Nuovo trattore (nuovi)</h1>
      ...
      <form onSubmit={submit} className="space-y-4">
        ...
        <div className="grid grid-cols-3 gap-3">
          <div className="col-span-2">
            ...
            <input className="w-full border rounded px-3 py-2" value={form.photo} onChange={e=>setF("photo", e.target.value)} placeholder="https://..." />
          </div>
          <div>
        ...
        <button disabled={saving} className="rounded bg-black text-white px-4 py-2 disabled:opacity-50">{saving ? "Salvo…" : "Crea"}</button>
      </form>
    </div>
  );
}
