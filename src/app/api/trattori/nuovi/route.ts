...
export async function GET(req: Request) {
  ...
  const r = await backend(`/api/trattori/nuovi${qs.toString() ? "?" + qs : ""}`, { cache: "no-store" as any });
...
export async function POST(req: Request) {
  ...
  const r = await backend("/api/trattori/nuovi", { method: "POST", body: JSON.stringify(body) });
  const data = await jsonOrText(r);
  return NextResponse.json(data, { status: r.status });
}
